...
public class AudioViewBottomSheetDailogFragment extends BottomSheetDialogFragment {
    ...
    private EPubReaderActivity mEPubReaderActivity;
    ...
    private void initViews() {
        ...
        mEPubReaderActivity = (EPubReaderActivity) mHalfSpeed.getContext();
            ...
            public void run() {
                ...
                if (mPlayer.getDuration() != currentPosition) {
                    if (currentPosition > mEnd) {
                        mAudioElement = mEPubReaderActivity.getElement(mPosition);
                        mEnd = (int) mAudioElement.getClipEnd();
                        //if(isAdded()) {
                        EPubReaderActivity.BUS.post(mPosition);
                        //mEPubReaderActivity.setHighLight(mPosition);
                        // }
                        mPosition++;
                    }
                ...
                mTextToSpeech.setOnUtteranceCompletedListener(
                        new TextToSpeech.OnUtteranceCompletedListener() {
                            ...
                            public void onUtteranceCompleted(String utteranceId) {
                                mEPubReaderActivity.runOnUiThread(new Runnable() {

                                    @Override
                                    public void run() {
                                        if (mIsSpeaking) {
                                             EPubReaderActivity.BUS.post(true);
                                            //mEPubReaderActivity.getSentance();
                                        }
                                    }
                                });
                            }
        ...
        mPlayPauseBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mEPubReaderActivity.isSmilAvailable()) {
                    playAudio();
                } else {
        ...
        mBackgroundColorStyle.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //mEPubReaderActivity.setHighLightStyle(mHighlightStyle);
                EPubReaderActivity.BUS.post(mHighlightStyle);
    ...
    private void playAudioWithoutSmil() {
        if (mTextToSpeech.isSpeaking()) {
            mTextToSpeech.stop();
            mIsSpeaking = false;
            EPubReaderActivity.BUS.post(new RewindIndex());
            //mEPubReaderActivity.resetCurrentIndex();
    ...
    private void playAudio() {
        ...
        if (mPlayer.isPlaying()) {
            ...
            mHandler.removeCallbacks(mHighlightTask);
            UiUtil.keepScreenAwake(false, mContext);
            //mEPubReaderActivity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        } else {
            if (mHandler == null) {
                mHandler = new Handler();
            }

            mEPubReaderActivity.setPagerToPosition(mPosition);
            mPlayer.start();
            mPlayPauseBtn.setImageDrawable(getResources().getDrawable(R.drawable.pause_btn));
            mHandler.post(mHighlightTask);
            UiUtil.keepScreenAwake(true, mContext);
            //mEPubReaderActivity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        }
    ...
    public void unRegisterBus() {
        if(isRegistered) {
            EPubReaderActivity.BUS.unregister(this);
        }
    }

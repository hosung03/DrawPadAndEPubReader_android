...
public class FileUtil {
    ...
    private static final String EPUB_READER_ROOT = "/epubreader/";

    public static Book saveEpubFile(final Context context, EPubReaderActivity.EpubSourceType epubSourceType, String epubFilePath, int epubRawId, String epubFileName) {
        String filePath;
        ...
        try {
            ...
            filePath = getEpubFilePath(epubSourceType, epubFilePath, epubFileName);

            if (!isFolderAvalable) {
                if (epubSourceType.equals(EPubReaderActivity.EpubSourceType.RAW)) {
                    epubInputStream = context.getResources().openRawResource(epubRawId);
                    saveTempEpubFile(filePath, epubFileName, epubInputStream);
                } else if (epubSourceType.equals(EPubReaderActivity.EpubSourceType.ASSESTS)) {
    ...
    public static String getEpubFolderPath(String epubFileName) {
        return Environment.getExternalStorageDirectory().getAbsolutePath()
                + "/" + EPUB_READER_ROOT + "/" + epubFileName;
    }

    public static String getEpubFilePath(EPubReaderActivity.EpubSourceType sourceType, String epubFilePath, String epubFileName) {
        if (EPubReaderActivity.EpubSourceType.SD_CARD.equals(sourceType)) {
            return epubFilePath;
        } else {
            return getEpubFolderPath(epubFileName) + "/" + epubFileName + ".epub";
        }
    }

    private static boolean isFolderAvailable(String epubFileName) {
        File file = new File(getEpubFolderPath(epubFileName));
        return file.isDirectory();
    }

    public static String getEpubFilename(Context context, EPubReaderActivity.EpubSourceType epubSourceType,
    ...
    public static Boolean saveTempEpubFile(String filePath, String fileName, InputStream inputStream) {
        ...
        try {
            if (!file.exists()) {
                File folder = new File(getEpubFolderPath(fileName));
                folder.mkdirs();
